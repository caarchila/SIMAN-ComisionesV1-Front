...
const SimpleModal = ({
  ...
  useEffect(() => {
    ...
    getProceso(process).then((data) => {
        console.log(data);
    });

    if (isOpen && process) {
      setSelectedChain(process.cadena || "");
      setSelectedCountry(process.pais || "");
      setStartDate(process.initialDate || "");
      setEndDate(process.endDate || "");
      setProcessDate(process.processDate || "");
    } else if (!isOpen) {
      setSelectedChain("");
      setSelectedCountry("");
      setStartDate("");
  ...
  const handleSubmit = (e) => {
    ...
    if (areDatesInSameMonth(startDate, endDate)) {
      ...
      showToast("Dates are not in the same month", "error");
    }
  };
            ...
            <div className="p-2 flex items-center justify-between">
              ...
              >
                <option value="">--Seleccione--</option>
                 <option value="8">Almacenes Simán S.A. de C.V. El Salvador</option>
              </select>
            </div>
